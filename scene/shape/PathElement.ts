...
export abstract class PathElement {
    ...
    abstract addTo(pgPath: NSVGPath): void;
...
export class QuadCurveTo extends PathElement {
    ...
    addTo(pgPath: NSVGPath): void {
        if (this.isAbsolute()) {
            ...
            pgPath.addQuadTo((this.getControlX() + dx), (this.getControlY() + dy), (this.getX() + dx), (this.getY() +
                                                                                                        dy));
        }
    }
...
export class CubicCurveTo extends PathElement {
    ...
    addTo(pgPath: NSVGPath): void {
        if (this.isAbsolute()) {
            ...
            let dx: number = pgPath.currentX;
            let dy: number = pgPath.currentY;
            pgPath.addCubicTo((this.ctrlX1 + dx), (this.ctrlY1 + dy), (this.ctrlX2 + dx), (this.ctrlY2 + dy), (this.x +
                                                                                                               dx), (this.y +
                                                                                                                     dy));
        }
    }
    ...
    toString(): string {
        return " C " + this.ctrlX1 + "," + this.ctrlY1 + "," + this.ctrlX2 + "," + this.ctrlY2 + "," + this.x + "," +
               this.y + " ";
    }
}
...
export class ArcTo extends PathElement {
    ...
    constructor();
    constructor(radiusX: number, radiusY: number, xAxisRotation: number, x: number, y: number, largeArcFlag: boolean,
                sweepFlag: boolean);
    constructor(radiusX?: number, radiusY?: number, xAxisRotation?: number, x?: number, y?: number,
    ...
    addArcTo(pgPath: NSVGPath, x0: number, y0: number): void {
        ...
        let xAxisRotationR: number = Utils.toRadians(this.xAxisRotation);
        ...
        sign                   = ((uy < 0.0) ? -1.0 : 1.0);
        let angleStart: number = Utils.toDegrees(sign * Math.acos(p / n));

        // Compute the angle extent
        n                       = Math.sqrt((ux * ux + uy * uy) * (vx * vx + vy * vy));
        p                       = ux * vx + uy * vy;
        sign                    = ((ux * vy - uy * vx < 0.0) ? -1.0 : 1.0);
        let angleExtent: number = Utils.toDegrees(sign * Math.acos(p / n));
    ...
    toString(): string {
        return " A " + this.radiusX + "," + this.radiusY + "," + this.xAxisRotation + "," +
               this.largeArcFlag ? "1" : "0" + "," + this.sweepFlag ? "1" : "0" + "," + this.x + "," + this.y + " ";
    }
}
...
>>>>>>> 95f64cb789e449ff98aa1d56795b46f14155648d
