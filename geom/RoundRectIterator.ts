...
import {RoundRectangle2D} from "./RoundRectangle2D";
import WindingRule = geom.WindingRule;

export class RoundRectIterator implements PathIterator{
    x         : number;
    y         : number;
    w         : number;
    h         : number;
    ...
    constructor(rr : RoundRectangle2D, tx : BaseTransform) {
        this.x  = rr.x;
        this.y  = rr.y;
        this.w  = rr.width;
        this.h  = rr.height;
