...
import PathIterator = geom.PathIterator;
import SegmentType = geom.SegmentType;
import {Dimension2D} from "../geometry/Dimension2D";
...
export abstract class Shape {
    ...
    abstract getPathIterator(tx: BaseTransform): PathIterator;

    abstract getPathIterator(tx: BaseTransform, flatness: number): PathIterator;
    ...
    static pointCrossingsForPath(pi: PathIterator, px: number, py: number): number {
        ...
        let coords: number[] = [];
        if (pi.currentSegment(coords) != SegmentType.MOVE_TO) {
    ...
    static accumulate(bbox: number[], s: Shape, tx: BaseTransform): void {
        let pi: PathIterator = s.getPathIterator(tx);
        let coords: number[]      = [];
...
export abstract class RectangularShape extends Shape {
    ...
    setFrame(loc: Point2D, size: Dimension2D): void {
    ...
    setFrameFromDiagonal(p1: Point2D, p2: Point2D): void {
