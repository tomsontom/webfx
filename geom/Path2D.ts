...
import SegmentType = geom.SegmentType;
...
export class TxIterator extends Iterator {
    ...
    constructor(p2df : Path2D, tx : BaseTransform) {
        super(p2df);
        this.floatCoords = p2df.floatCoords;
        this.transform   = tx;
    }
...
export class Path2D extends Shape implements PathConsumer2D {
    ...
    checkAndGetIntRect(retrect: Rectangle, tolerance: number): boolean {
        ...
        if (this.numTypes == 5) {
            if (this.pointTypes[4] != SegmentType.LINE_TO && this.pointTypes[4] != SegmentType.CLOSE) {
                return false;
            }
        } else if (this.numTypes == 6) {
            if (this.pointTypes[4] != SegmentType.LINE_TO) {
                return false;
            }
            if (this.pointTypes[5] != SegmentType.CLOSE) {
                return false;
            }
        } else if (this.numTypes != 4) {
            return false;
        }
        if (this.pointTypes[0] != SegmentType.MOVE_TO) {
            return false;
        }
        if (this.pointTypes[1] != SegmentType.LINE_TO) {
            return false;
        }
        if (this.pointTypes[2] != SegmentType.LINE_TO) {
            return false;
        }
        if (this.pointTypes[3] != SegmentType.LINE_TO) {
            return false;
        }
        ...
        if (this.numTypes > 4 && this.pointTypes[4] == SegmentType.LINE_TO) {
    ...
    moveTo(x : number, y : number): void {
        if (this.numTypes > 0 && this.pointTypes[this.numTypes - 1] == SegmentType.MOVE_TO) {
            ...
            this.pointTypes[this.numTypes++]   = SegmentType.MOVE_TO;
    ...
    lineTo(x: number, y: number): void {
        ...
        this.pointTypes[this.numTypes++]   = SegmentType.LINE_TO;
    ...
    quadTo(xc: number, yc: number, x: number, y: number): void {
        ...
        this.pointTypes[this.numTypes++]   = SegmentType.QUAD_TO;
    ...
    curveTo(xc1: number, yc1: number, xc2: number, yc2: number, x: number, y: number): void {
        ...
        this.pointTypes[this.numTypes++]   = SegmentType.CURVE_TO;
    ...
    closePath(): void {
        if (this.numTypes == 0 || this.pointTypes[this.numTypes - 1] != SegmentType.CLOSE) {
            this.needRoom(true, 0);
            this.pointTypes[this.numTypes++] = SegmentType.CLOSE;
    ...
    rectCrossings(rxmin : number, rymin : number, rxmax : number, rymax: number) : number {
        ...
        let ci        : number = 2;
        for (let i : number = 1 ; crossings != Shape.RECT_INTERSECTS && i < this.numTypes; i++) {
            switch (this.pointTypes[i]) {
                ...
                case geom.SegmentType.LINE_TO:
                    crossings = Shape.rectCrossingsForLine(crossings, rxmin, rymin, rxmax, rymax, endx = coords[ci++], endy = coords[ci++]);
                    curx = endx;
                    cury = endy;
                    break;
                case geom.SegmentType.QUAD_TO:
                    crossings = Shape.rectCrossingsForQuad(crossings, rxmin, rymin, rxmax, rymax, curx, cury, coords[ci++], coords[ci++], endx = coords[ci++], endy = coords[ci++], 0);
                    curx = endx;
                    cury = endy;
                    break;
                case geom.SegmentType.CURVE_TO:
                    crossings = Shape.rectCrossingsForCubic(crossings, rxmin, rymin, rxmax, rymax, curx, cury, coords[ci++], coords[ci++], coords[ci++], coords[ci++], endx = coords[ci++], endy = coords[ci++], 0);
                    curx = endx;
                    cury = endy;
                    break;
    ...
    append(pi : PathIterator | Shape, connect : boolean) : void {
        if (pi instanceof Shape) {
        ...
        while (!pi.isDone()) {
            switch (pi.currentSegment(coords)) {
                case SegmentType.MOVE_TO:
                    ...
                    if (this.pointTypes[this.numTypes - 1] != SegmentType.CLOSE &&
    ...
    transform(tx : BaseTransform) : void {
    ...
    createTransformedShape(tx : BaseTransform) : Shape{
    ...
    static contains(pi : PathIterator, x? : number, y? : number, p? : Point2D) : boolean {
        if (p !== undefined && p instanceof Point2D) {
            x = p.x;
            y = p.y;
        }
        ...
        if (x * 0 + y * 0 == 0) {
            ...
            let cross: number = Shape.pointCrossingsForPath(pi, x, y);
            return ((cross & mask) != 0);
        } else {
            return false;
        }
    }
    static contains(pi : PathIterator, x : number, y : number, w : number, h : number) : boolean {
        ...
        let crossings : number = Shape.rectCrossingsForPath(pi, x, y, x + w, y + h);
        return (crossings != Shape.RECT_INTERSECTS && (crossings & mask) != 0);
    }

    contains(x? : number, y? : number, p? : Point2D) : boolean {
        if (p !== undefined && p instanceof Point2D) {
            x = p.x;
            y = p.y;
        }
    ...
    static intersects(pi : PathIterator, x : number, y : number, w : number, h : number) : boolean {
        if (Number.isNaN(x + w) || Number.isNaN(y + h)) {
            return false;
        }
        if (w <= 0 || h <= 0) {
    ...
    getPathIterator(tx : BaseTransform, flatness? : number) : PathIterator {
